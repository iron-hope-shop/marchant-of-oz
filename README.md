...
- `public/index.html`: Static entry point and interactive UI application with Top Nav, Home, Portal, Settings Drawer, and build details.
- `public/tamagui.css`: Zero-dependency design system with 12-step color scales, themes (Light/Dark), sub-themes (blue, purple, green, red, orange, pink, yellow), layout stacks (YStack, XStack, ZStack), and component styles.
- `public/tamagui-theme.js`: Vanilla theme runtime engine with persistence and responsive color-scheme listener.
- `public/version.json`: Internal build metadata (version, build number, date).
- `.github/workflows/google-cloudrun-docker.yml`: Automated CI/CD pipeline.
